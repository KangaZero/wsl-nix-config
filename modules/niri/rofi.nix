{ pkgs, ... }:
let
  # Catppuccin Mocha — same hexes as the niri theme.
  ...
  # actually runs (bound to $mod+Shift+e in modules/niri/default.nix).
  ...
  # Clipboard history applet ($mod+Shift+v).
  # cliphist daemon is managed by modules/niri/cliphist.nix.
  # wl-copy writes the decoded entry back to the Wayland clipboard.
  ...
  # the binds in modules/niri/default.nix.
...
{
  home.packages = [
    rofiPkg # rofi + calc plugin; launcher/window/combi/calc
    rofi-powermenu # powermenu ($mod+Shift+e)
    rofi-clipboard # clipboard history ($mod+Shift+v)
  ...
  xdg.configFile = {
    ...
    #      display-wayland = true forces the Wayland backend (rofi-wayland was
    #      merged into pkgs.rofi on nixpkgs-unstable; the flag selects it at
    #      runtime). ----
